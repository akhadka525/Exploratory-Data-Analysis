...
## from all sources grouping by year
...
## Now Plot the total Pm2.5 emissions for Baltimore from all sources for each year and save to plot2.png

{
        png("Plot2.png", width=720, height=720)
        
        par(mgp = c(2.2, 1, 0))
        barplot(total_emission_Baltimore$totalEmission/10^3,
                names.arg = total_emission_Baltimore$year,
                ## emission value is large so expressed in scale of 10^3 
                col = "Blue",
                xlab = "Year",
                ylab = expression(Total ~ PM[2.5] ~ Emissions ~ (10^3 ~ Tons), line = 0),
                main = expression(Total ~ PM[2.5] ~ Emissions ~ From ~ All ~ Sources ~
                                          For ~ Baltimore ~ City ~ Maryland),
                ylim = c(0, max(total_emission_Baltimore$totalEmission/10^3+1)))
